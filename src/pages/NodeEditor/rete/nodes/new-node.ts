...
export class NewNode extends ClassicPreset.Node<
...
> {
  ...
  constructor(initial: string, private dataflow: DataflowEngine<Schemes>) {
    ...
    this.addInput("text", new ClassicPreset.Input(new TextSocket(), "Type"));

    this.addOutput(
      "consequent",
      new ClassicPreset.Output(new ActionSocket(), "Action")
    );

    //this.addOutput("alternate", new ClassicPreset.Output(new TextSocket(), "Type"));
  }
